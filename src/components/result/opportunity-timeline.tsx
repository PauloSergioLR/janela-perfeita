...
export function OpportunityTimeline({ recommendation }: OpportunityTimelineProps) {
  ...
  return (
    ...
    >
      <CardHeader className="gap-1 border-b border-soft bg-weather-card p-2.5">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <p className="text-sm font-medium text-weather-accent">
              Oportunidade por hora
            </p>
            <CardTitle className="mt-0.5 text-base">Pontuação de oportunidade por hora</CardTitle>
        ...
        {data.length > 0 ? (
          <>
            <div className="overflow-x-auto pb-1 lg:overflow-visible">
              <div className="flex min-w-max items-end justify-between gap-1 px-1 pt-1 lg:min-w-0">
                {data.map((datum) => {
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        "group grid w-12 shrink-0 gap-1 rounded-lg border border-transparent p-1 text-center transition-[transform,border-color,background-color,box-shadow,opacity] duration-200 ease-out motion-safe:active:scale-[0.98] motion-reduce:transition-none focus-visible:outline-none focus-visible:ring-3 focus-visible:ring-ring/45 lg:w-auto lg:flex-1",
                    ...
                    >
                      <span className="flex h-12 items-end rounded-md border border-soft bg-muted/35 p-1">
                        <span
                          className={cn(
                      ...
                      <span className="grid gap-0.5 text-[10px] text-muted-foreground">
            ...
            {selectedDatum ? (
              <div
                className="border-t border-soft pt-2 lg:hidden"
                aria-live="polite"
